...
@Component({
  selector: 'app-exclusion-admin',
  templateUrl: './exclusion-admin.component.html',
  styleUrls: ['./exclusion-admin.component.css']
})
export class ExclusionAdminComponent implements OnInit  {
  ...
  constructor(private _ITOturbineConfigService: ITOturbineConfigService, private _ITOScopeOfSupplyService: ITOScopeOfSupplyService,
    ...
    private _ITOeditQoutService: ITOeditQoutService,  @Inject(LOCAL_STORAGE) private storage: WebStorageService,) {
      ...
      this._BgmratingsComponentService.getAdminCacheWithAIList().subscribe(resp => {
        console.log(resp);
        this.spinner=true;
        this.tableList=resp.dropDownColumnvalues.placeList17.placeList17;
        for(let j=0;j<this.tableList.length;j++)
        {
           ...
           this.cols = [
            ...
            { field: 'variantType', header: 'VariantType' },
...
update()
{
  ...
  this.adminForm.adminExclusionMastList.push(this.dboClass);
  }
  this._BgmratingsComponentService.getAdminExclusionMast(this.adminForm).subscribe(res => {
    console.log(res);
    this.adminForm = res;
