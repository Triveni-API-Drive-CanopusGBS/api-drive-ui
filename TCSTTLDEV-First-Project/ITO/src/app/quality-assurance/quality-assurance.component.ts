...
note:string=null;
 ...
 if(this._ITOeditQoutService.checkEdit == false){
    ...
    this.qualityLocal[this.quaility] = this.storage.get(this.quaility);
    if(this._ITOcustomerRequirementService.editFlag==true && this._ITOeditQoutService.qualitiasurance!=undefined)
{
  this.tempArray=this._ITOeditQoutService.qualitiasurance;
  this.mainSave=false;
  ...
  for(let x=0;x<this.tempArray.length;x++)
  {
    if(this.tempArray[x].newColValFlag==1)
    {
      ...
      this.dboClass.equipment=this.tempArray[x].equipment;
      this.dboClass.test=this.tempArray[x].test;
      ...
      {
       this.dboClass.remarks=this.data;
      }
     else
     {
    ...
    if (this.storage.get(this.quaility) == null) {
      this.saveInLocal(this.quaility, {
        othetList:this.othetList,itemOthersList1:this.itemOthersList1,arraycheck:this.arraycheck,data:this.data
      });
    }
  

       if(this.storage.get(this.quaility).data!=null && this.storage.get(this.quaility).data!="" ){
        this.data= this.storage.get(this.quaility).data;
      
     }
    if(this.storage.get(this.quaility).othetList.length!=0){
      this.othetList= this.storage.get(this.quaility).othetList;
      ...
      this.arraycheck= this.storage.get(this.quaility).arraycheck;
      this.mainSave=false;

   }
   if(this.storage.get(this.quaility).itemOthersList1.length!=0){
    this.itemOthersList1= this.storage.get(this.quaility).itemOthersList1;
    ...
    if(this.othetList.length==0)
    {
        ...
        console.log(this.storage.get(this.quaility));
        this._ItoPerformanceService.getOtherChapter(this.dboFormDataaa).subscribe(res => {
         ...
         {
          if(res.otherDataList[x].scopeCd=="QA")
          {
...
if(this.tempArray[y].seqNo==this.othetList[x].seqNo && this.tempArray[y].itemId==this.othetList[x].itemId )
{
  ...
  break;
              ...
              if(this.tempArray[0].remarks!=null && this.tempArray[0].remarks!="")
              {
                this.data=this.tempArray[0].remarks;
              }
...
this.arraycheck=[];
  ...
  if (!bolleancheck)
  {
    for(let x=0;x<this.othetList.length;x++)
    {
      ...
      {
      this.arraycheck[this.othetList[x].seqNo]=true;
      }
    }  }
  else
  {
    for(let x=0;x<this.tempArray.length;x++)
    {
      if(this.tempArray[x].itemId!=0 && this.tempArray[x].newColValFlag==0  )
      {
      this.arraycheck[this.tempArray[x].seqNo]=true;
      }
    }
  }
  ...
  saveInLocal(key, val): void {
    ...
    this.qualityLocal[key] = this.storage.get(key);
  }
  saveOthers()
  {
    this.getPrice=true;
    if(this.disablequaility.includes(true) )
    {
      this.getPrice=false;
    }
    if(this.getPrice )
    ...
    {
      if(this.arraycheck[this.othetList[j].seqNo]==true)
      {
      this.dboClass=new dboClass();
  ...
  itemRmrkCheck(event,no)
  {
    this.mainSave=true;

    if(event.target.checked){
     this.arraycheck[no]=true;
    }else if(!event.target.checked){
      this.arraycheck[no]=false;
    
    }
  }
  compOthersForm1(others, othersfrm: NgForm,name) {
    this.mainSave=true;

    this.dboClass=new dboClass();
      this.dboClass.seqNo=0;
      this.dboClass.ssId=this.othetList[0].ssId;
      this.dboClass.itemId=this.othetList[0].itemId;
      this.dboClass.subItemId=this.othetList[0].subItemId;
      this.dboClass.information= null;
      this.dboClass.finalts=null;
      this.dboClass.subItemCd=this.othetList[0].subItemCd;
      this.dboClass.description= null;
      this.dboClass.equipment=others.othdes1;
      this.dboClass.test=others.othdes2;
      this.dboClass.equivalent=null;
      this.dboClass.panelType	=null;
      this.dboClass.custType=null;
      this.dboClass.quant=null;
  ...
  cancelLinesOth1(i) {
    ...
    this.disablequaility.splice(i,1);
