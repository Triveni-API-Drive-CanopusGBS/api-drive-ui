...
@Component({
  selector: 'app-tender-drawings',
  templateUrl: './tender-drawings.component.html',
  styleUrls: ['./tender-drawings.component.css']
})
export class TenderDrawingsComponent implements OnInit {

  dboFormDataaa: any; //To store dbo form response
  quotId:number=0;
  tender: string = 'tender'; // local storage value
  tenderLocal: Array<any> = [];
  display: Array<any> = [];
  dboClass: dboClass;
  othetList: Array<any> = [];
 ...
 getPrice:boolean=true;
  constructor(private _ItoPerformanceService: ItoPerformanceService,private _ITOLoginService: ITOLoginService, private _ITOeditQoutService: ITOeditQoutService,  private _ITOcustomerRequirementService: ITOcustomerRequirementService,
    ...
    this.tenderLocal[this.tender] = this.storage.get(this.tender);
    if(this._ITOcustomerRequirementService.editFlag==true && this._ITOeditQoutService.tenderDraw!=undefined)
{
  this.tempArray=this._ITOeditQoutService.tenderDraw;
  this.mainSave=false;
  ...
  for(let x=0;x<this.tempArray.length;x++)
  {
    if(this.tempArray[x].newColValFlag==1)
    {
      ...
      this.dboClass.information= this.tempArray[x].information;
      this.dboClass.finalts=this.tempArray[x].finalts;
    ...
    if (this.storage.get(this.tender) == null) {
      this.saveInLocal(this.tender, {
        othetList:this.othetList,itemOthersList1:this.itemOthersList1,arraycheck:this.arraycheck
      });
    }
    if(this.storage.get(this.tender).othetList.length!=0){
      this.othetList= this.storage.get(this.tender).othetList;
      ...
      this.arraycheck= this.storage.get(this.tender).arraycheck;
      this.mainSave=false;

   }
   if(this.storage.get(this.tender).itemOthersList1.length!=0){
    this.itemOthersList1= this.storage.get(this.tender).itemOthersList1;
    ...
    if(this.othetList.length==0)
    {
        ...
        console.log(this.storage.get(this.tender));
        this._ItoPerformanceService.getOtherChapter(this.dboFormDataaa).subscribe(res => {
         ...
         {
          if(res.otherDataList[x].scopeCd=="DR")
          {
...
if(this.tempArray[y].seqNo==this.othetList[x].seqNo && this.tempArray[y].itemId==this.othetList[x].itemId )
{
  ...
  saveInLocal(key, val): void {
    ...
    this.tenderLocal[key] = this.storage.get(key);
  }
  saveOthers()
  {
    this.getPrice=true;
    if(this.disabletender.includes(true) )
    {
      this.getPrice=false;
    }
    if(this.getPrice )
  ...
  compOthersForm1(others, othersfrm: NgForm,id,name) {
    ...
    this.dboClass=new dboClass();
      ...
      this.dboClass.information= others.othdes2;
      this.dboClass.finalts=others.othdes3;
