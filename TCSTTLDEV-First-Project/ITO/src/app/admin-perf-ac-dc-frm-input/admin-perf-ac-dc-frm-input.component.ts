...
@Component({
  selector: 'app-admin-perf-ac-dc-frm-input',
  templateUrl: './admin-perf-ac-dc-frm-input.component.html',
  styleUrls: ['./admin-perf-ac-dc-frm-input.component.css']
})
export class AdminPerfAcDcFrmInputComponent implements OnInit {
  ...
  constructor(private _ITOturbineConfigService: ITOturbineConfigService, private _ITOScopeOfSupplyService: ITOScopeOfSupplyService,
    ...
    private _ITOeditQoutService: ITOeditQoutService,  @Inject(LOCAL_STORAGE) private storage: WebStorageService,) {
      ...
      this._BgmratingsComponentService.getAdminCacheWithAIList().subscribe(resp => {
        this.spinner=true;
        console.log(resp);
        this.tableList=resp.dropDownColumnvalues.placeList11.placeList11;
        for(let j=0;j<this.tableList.length;j++)
        {


           this.cols = [
            { field: 'frmName', header: 'frmName' },
            { field: 'minPower', header: 'minPower' },
            { field: 'maxPower', header: 'maxPower' },
...
update()
{
  ...
  this.dboClass.frmId=this.finalFrameList[j].frmId;
  this.dboClass.itemCd=this.finalFrameList[j].itemCd;
  this.dboClass.minPower=this.finalFrameList[j].minPower;
  this.dboClass.maxPower=this.finalFrameList[j].maxPower;//error
  ...
  this.adminForm.adminPerfAcDcFrmInputList.push(this.dboClass);
  }
  this._BgmratingsComponentService.getAdminPerfAcDcFrmInput(this.adminForm).subscribe(res => {
    console.log(res);
    this.adminForm = res;
