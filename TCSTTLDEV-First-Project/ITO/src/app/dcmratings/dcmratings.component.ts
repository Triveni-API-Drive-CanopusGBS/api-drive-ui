...
@Component({
  selector: 'app-dcmratings',
  templateUrl: './dcmratings.component.html',
  styleUrls: ['./dcmratings.component.css']
})
export class DcmratingsComponent implements OnInit {
  ...
  { 
    ...
    this._BgmratingsComponentService.getAdminCacheWithAIList().subscribe(resp => {
      this.spinner=true;
      console.log(resp);
      this.tableList=resp.dropDownColumnvalues.placeList5.placeList5;
      for(let j=0;j<this.tableList.length;j++)
      {
         ...
         this.cols = [
          ...
          { field: 'eopMotorRating', header: 'EOP Motor Rating' },
          { field: 'cost', header: 'Cost' },
          { field: 'pumpType', header: 'PumpType' },
          { field: 'bgHrRate', header: 'BGHR Rate' },
          { field: 'dcOutputVol', header: 'DC Output Voltage' },
...
update()
{
  ...
  // bean.getActiveNew()
  this.adminForm.adminDcmPowerCalcList=[];
  for(let j=0;j<this.finalFrameList.length;j++)
  {
  ...
  this.dboClass.eopMotorRating=this.finalFrameList[j].eopMotorRating;
  this.dboClass.cost=this.finalFrameList[j].cost;
  this.dboClass.pumpType=this.finalFrameList[j].pumpType;
  this.dboClass.bgHrRate=this.finalFrameList[j].bgHrRate;
  this.dboClass.dcOutputVol=this.finalFrameList[j].dcOutputVol;
  this.dboClass.defaultFlagNew=this.finalFrameList[j].defaultFlag; // error
  this.dboClass.activeNew=this.finalFrameList[j].activeNew;// not there
  this.adminForm.adminDcmPowerCalcList.push(this.dboClass);
  }
  this._BgmratingsComponentService.getAdminDcmPowerCalc(this.adminForm).subscribe(res => {
    console.log(res);
    this.adminForm = res;
