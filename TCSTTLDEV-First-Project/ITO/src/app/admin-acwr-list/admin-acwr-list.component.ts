...
@Component({
  selector: 'app-admin-acwr-list',
  templateUrl: './admin-acwr-list.component.html',
  styleUrls: ['./admin-acwr-list.component.css']
})
export class AdminAcwrListComponent implements OnInit {
  ...
  constructor(private _ITOturbineConfigService: ITOturbineConfigService, private _ITOScopeOfSupplyService: ITOScopeOfSupplyService,
    ...
    private _ITOeditQoutService: ITOeditQoutService,  @Inject(LOCAL_STORAGE) private storage: WebStorageService,) {
      ...
      this._BgmratingsComponentService.getAdminCacheWithAIList().subscribe(resp => {
        console.log(resp);
        this.spinner=true;
        this.tableList=resp.dropDownColumnvalues.placeList20.placeList20;
        for(let j=0;j<this.tableList.length;j++)
        {


           this.cols = [
            { field: 'itemCd', header: 'Item Cd' },
            { field: 'coolingMethod', header: 'Cooling Method' },
            { field: 'tempRaise', header: 'Temperature Raise' },
			{ field: 'minPower', header: 'Min Power' },
			{ field: 'maxPower', header: 'Max Power' },
			{ field: 'turbOilCooler', header: 'Turbine Oil Cooler' },
            {field: 'generatorCooler', header: 'Generator Cooler'},
            {field: 'glandVendCond', header: 'Gland Vend Cond'},
...
update()
{
 ...
 this.dboClass.id=this.finalFrameList[j].id;
				  this.dboClass.itemCd=this.finalFrameList[j].itemCd;
				   this.dboClass.coolingMethod=this.finalFrameList[j].coolingMethod;
					 this.dboClass.tempRaise=this.finalFrameList[j].tempRaise;
					  this.dboClass.minPower=this.finalFrameList[j].minPower;
					   this.dboClass.maxPower=this.finalFrameList[j].maxPower;
              this.dboClass.turbOilCooler=this.finalFrameList[j].turbOilCooler;
              this.dboClass.generatorCooler=this.finalFrameList[j].generatorCooler;
              this.dboClass.glandVendCond=this.finalFrameList[j].glandVendCond;
						 this.dboClass.activeNew=1;

  this.adminForm.adminAcwrList.push(this.dboClass);
  }
  this._BgmratingsComponentService.getAdminAcwr(this.adminForm).subscribe(res => {
    console.log(res);
    this.adminForm = res;
