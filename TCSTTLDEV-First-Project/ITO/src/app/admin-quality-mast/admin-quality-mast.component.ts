...
@Component({
  selector: 'app-admin-quality-mast',
  templateUrl: './admin-quality-mast.component.html',
  styleUrls: ['./admin-quality-mast.component.css']
})
export class AdminQualityMastComponent implements OnInit {
  ...
  constructor(private _ITOturbineConfigService: ITOturbineConfigService, private _ITOScopeOfSupplyService: ITOScopeOfSupplyService,
    ...
    private _ITOeditQoutService: ITOeditQoutService,  @Inject(LOCAL_STORAGE) private storage: WebStorageService,) {
      ...
      this._BgmratingsComponentService.getAdminCacheWithAIList().subscribe(resp => {
        console.log(resp);
        this.spinner=true;
        this.tableList=resp.dropDownColumnvalues.placeList15.placeList15;
				   ...
				   this.dboClass.subScopeCd=this.finalFrameList[j].subScopeCd;
					 this.dboClass.panelType=this.finalFrameList[j].panelType;
					  this.dboClass.equipment=this.finalFrameList[j].equipment;
					   this.dboClass.test=this.finalFrameList[j].test;
              this.dboClass.editFlag=this.finalFrameList[j].editFlag;
						 this.dboClass.activeNew=1;

  this.adminForm.adminQualityMastList.push(this.dboClass);
  }
  this._BgmratingsComponentService.getAdminQualityMast(this.adminForm).subscribe(res => {
    console.log(res);
    this.adminForm = res;
