...
import { ItoUpdateCostDBOElecPrice } from './update-cost-dboelect-price.service';
import { DBOElecPrice } from './update-cost-dboelect-price';
import { DBOMechPrice } from '../update-cost-dbomech-price/update-cost-dbomech-price';

@Component({
  selector: 'app-update-cost-dboelect-price',
  templateUrl: './update-cost-dboelect-price.component.html',
  styleUrls: ['./update-cost-dboelect-price.component.css']
})

export class UpdateCostDboelectPriceComponent implements OnInit {
  ...
  data: Array<any> = [];
  tempPriceCode: Array<any> = [];   // local variable to store price code
  colName: Array<any> = [];
  ...
  constructor(private _ItoUpdateCostDBOElecPrice: ItoUpdateCostDBOElecPrice, private _ITOLoginService: ITOLoginService,
    ...
    this._ItoUpdateCostDBOElecPrice.getFrameAndUserData().subscribe(res => {
      ...
      if (this._ITOMyWorkListPageService.selectedUR) {
        ...
        if (this.selectedUR.updateCode == "UPD_DBO_ELE_PRICE_NEW") {
          this.exist = true;
          this.tabIndex = 1;
          this.addNew = false;
        } else if (this.selectedUR.updateCode == "UPD_DBO_ELE_PRICE") {
          this.addNew = true;
          this.exist = false;
          this.tabIndex = 0
        }
        ...
        if (this.storage.get(this.currentRole) == "DBO_ELE_EDIT") {
          ...
          this.panelCode = this._ITOMyWorkListPageService.responseTemp.savedDboEleList[0].panelCode;
          this.itemId = this._ITOMyWorkListPageService.responseTemp.savedDboEleList[0].itemId;
          this.itemName = this._ITOMyWorkListPageService.responseTemp.savedDboEleList[0].itemName;
          this.custCode = this._ITOMyWorkListPageService.responseTemp.savedDboEleList[0].custCode;  // change this after adding in db
          if (this.custCode == "DM") {
          ...
          for (let i = 0; i < this._ITOMyWorkListPageService.responseTemp.unsavedDboEleList.length; i++) {
            for (let p = 0; p < this._ITOMyWorkListPageService.responseTemp.savedDboEleList.length; p++) {
              if (this._ITOMyWorkListPageService.responseTemp.unsavedDboEleList[i].itemId == this._ITOMyWorkListPageService.responseTemp.savedDboEleList[p].itemId) {
                this.DBOData.push(this._ITOMyWorkListPageService.responseTemp.unsavedDboEleList[i]);
              }
            }
          }
          // get selected data from savedDboEleList
          this.finalRangeList = this._ITOMyWorkListPageService.responseTemp.savedDboEleList;
        ...
        // if current role is Reviewer/Approver this else if will be exceuted
          ...
          this.panelCode = this._ITOMyWorkListPageService.responseTemp.savedDboEleList[0].panelCode;
          this.panelCdNew = this._ITOMyWorkListPageService.responseTemp.savedDboEleList[0].panelCode;
          this.itemId = this._ITOMyWorkListPageService.responseTemp.savedDboEleList[0].itemId;
          this.itemName = this._ITOMyWorkListPageService.responseTemp.savedDboEleList[0].itemName;
          this.itemNameNew = this._ITOMyWorkListPageService.responseTemp.savedDboEleList[0].itemName;
          this.custCode = this._ITOMyWorkListPageService.responseTemp.savedDboEleList[0].custCode;  // change this after adding in db
          if (this.custCode == "DM") {
            this.custType = "Domestic";
          } else if (this.custCode == "EX") {
          ...
          this.DBOData = this._ITOMyWorkListPageService.responseTemp.savedDboEleList;
          this.finalRangeList = this._ITOMyWorkListPageService.responseTemp.savedDboEleList;
          if (this.DBOData.length > 0) {
            this.tableDiv = true;
          }
  ...
  getItemDetails(val) {
    ...
    this._ItoUpdateCostDBOElecPrice.getDBOElectricalData(val).subscribe(res => {
      console.log(res);
      this.tempRes = res;
      this.dBOElectricalListArray = res.dBOElectricalList;
    })
  }
  ...
  onRowSelect(event) {
    ...
    //getting electrical component column names
    this._ItoUpdateCostDBOElecPrice.getDBOEleUpdateColData(this.itemId, this.panelCode).subscribe(colRes => {
      console.log(colRes);
      this.colData = colRes.dBOElectricalColList;

      this.tempPriceCode = [];
      this.displayDialog = true;
      this.selectedRange = event.data;
      // splitting the price code to display colname and value
      this.tempPriceCode = this.selectedRange.priceCode.split(['|']);

      console.log(this.tempPriceCode)
      for (let k = 0; k < this.colData.length; k++) {
        for (let i = 0; i < this.tempPriceCode.length; i++) {
          if (this.tempPriceCode[i] == this.colData[k].colValCd) {
            this.colName.push(this.colData[k].colNm);
          }
        }
      }
  ...
  SaveAsDraft() {
    ...
    for (let q = 0; q < this.finalRangeList.length; q++) {
      if (this.finalRangeList[q].priceId > 0) {
        this.saveBasic.updateCode = "UPD_DBO_ELE_PRICE";
      } else {
        this.saveBasic.updateCode = "UPD_DBO_ELE_PRICE_NEW";
      }
    }
    ...
    this._ItoUpdateCostDBOElecPrice.createDboEleUpdateRequestPrice(this.tempRes1).subscribe(resp => {
      console.log(resp);
  ...
  addCost(addCostForm) {
    ...
    this._ItoUpdateCostDBOElecPrice.getDBOEleUpdatePriceData(this.tempRes).subscribe(resp1 => {
      ...
      if (k) {
        ...
        let m = this.finalRangeList.map((x) => x.priceCode).includes((eleData.priceCode));
        console.log(m);
        if (m) {
          alert("Data Already Exists");
        } else {
