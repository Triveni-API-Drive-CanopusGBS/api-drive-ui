...
@Component({
  selector: 'app-sub-supplier-list',
  templateUrl: './sub-supplier-list.component.html',
  styleUrls: ['./sub-supplier-list.component.css']
})
export class SubSupplierListComponent implements OnInit {

  dboFormDataaa: any; //To store dbo form response
  quotId:number=0;
  supplier: string = 'supplier'; // local storage value
  supplierLocal: Array<any> = [];
  display: Array<any> = [];
  dboClass: dboClass;
  othetList: Array<any> = [];
  symbol:string="*";
 ...
 mainSave:boolean=true;
  ...
  constructor(private _ItoPerformanceService: ItoPerformanceService,private _ITOLoginService: ITOLoginService, private _ITOeditQoutService: ITOeditQoutService,  private _ITOcustomerRequirementService: ITOcustomerRequirementService,
    ...
    this.supplierLocal[this.supplier] = this.storage.get(this.supplier);
    if(this._ITOcustomerRequirementService.editFlag==true && this._ITOeditQoutService.subSuppliersList!=undefined)
{
  this.tempArray=this._ITOeditQoutService.subSuppliersList;
  this.mainSave=false;
  ...
  for(let x=0;x<this.tempArray.length;x++)
  {
    if(this.tempArray[x].newColValFlag==1)
    {
      ...
      this.dboClass.equivalent=this.tempArray[x].equivalent;
    ...
    if (this.storage.get(this.supplier) == null) {
      this.saveInLocal(this.supplier, {
        othetList:this.othetList,itemOthersList1:this.itemOthersList1,arraycheck:this.arraycheck
      });
    }
    if(this.storage.get(this.supplier).othetList.length!=0){
      this.othetList= this.storage.get(this.supplier).othetList;
      ...
      this.arraycheck= this.storage.get(this.supplier).arraycheck;
      this.mainSave=false;

   }
   if(this.storage.get(this.supplier).itemOthersList1.length!=0){
    this.itemOthersList1= this.storage.get(this.supplier).itemOthersList1;
    ...
    if(this.othetList.length==0)
    {
        ...
        console.log(this.storage.get(this.supplier));
        this._ItoPerformanceService.getOtherChapter(this.dboFormDataaa).subscribe(res => {
         ...
         {
          if(res.otherDataList[x].scopeCd=="SSL")
          {
...
if(this.tempArray[y].seqNo==this.othetList[x].seqNo && this.tempArray[y].itemId==this.othetList[x].itemId )
{
  ...
  saveInLocal(key, val): void {
    ...
    this.supplierLocal[key] = this.storage.get(key);
  }
  saveOthers()
  {
    this.getPrice=true;
    if(this.disablesupplier.includes(true) )
    {
      this.getPrice=false;
    }
    if(this.getPrice )
  ...
  compOthersForm1(others, othersfrm: NgForm,id,name) {
    ...
    this.dboClass=new dboClass();
      ...
      this.dboClass.test=null;
      this.dboClass.equivalent=others.othdes2;
      this.dboClass.panelType	=null;
      this.dboClass.custType=null;
      this.dboClass.quant=null;
      this.dboClass.cost=null;
  ...
  cancelLinesOth1(i,id) {
    ...
    this.disablesupplier.splice(i,1);
