...
@Component({
  selector: 'app-admin-comr-month',
  templateUrl: './admin-comr-month.component.html',
  styleUrls: ['./admin-comr-month.component.css']
})
export class AdminComrMonthComponent implements OnInit {
  ...
  constructor(private _ITOturbineConfigService: ITOturbineConfigService, private _ITOScopeOfSupplyService: ITOScopeOfSupplyService,
    ...
    private _ITOeditQoutService: ITOeditQoutService,  @Inject(LOCAL_STORAGE) private storage: WebStorageService,) {
      ...
      this._BgmratingsComponentService.getAdminCacheWithAIList().subscribe(resp => {
        console.log(resp);
        this.spinner=true;
        this.tableList=resp.dropDownColumnvalues.placeList21.placeList21;
...
update()
{
 ...
 this.dboClass.id=this.finalFrameList[j].id;
				   this.dboClass.turbineCode=this.finalFrameList[j].turbineCode;
					 this.dboClass.turbineDesign=this.finalFrameList[j].turbineDesign;
  ...
  this.adminForm.adminComrMonthsList.push(this.dboClass);
  }
  this._BgmratingsComponentService.getAdminComrMonths(this.adminForm).subscribe(res => {
    console.log(res);
    this.adminForm = res;
