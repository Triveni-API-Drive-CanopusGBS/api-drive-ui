...
@Injectable()
export class ITOAddNewQuestionsService{
    ...
    private _getNewFramesForQuestions: string = "http://10.0.0.4:8082/ITO/admin/getNewFramesForQuestions";
  ...
  answerbeanList:any;
    ...
    getNewFramesForQuestions(): Observable<any> {
        let headers = new Headers({ 'Content-Type': 'application/json' });
            let options = new RequestOptions({ headers: headers });
            return this._http.post(this._getNewFramesForQuestions,'', options).map(this.extractData);
    }
