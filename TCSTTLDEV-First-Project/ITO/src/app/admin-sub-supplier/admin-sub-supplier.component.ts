...
@Component({
  selector: 'app-admin-sub-supplier',
  templateUrl: './admin-sub-supplier.component.html',
  styleUrls: ['./admin-sub-supplier.component.css']
})
export class AdminSubSupplierComponent implements OnInit {
  ...
  constructor(private _ITOturbineConfigService: ITOturbineConfigService, private _ITOScopeOfSupplyService: ITOScopeOfSupplyService,
    ...
    private _ITOeditQoutService: ITOeditQoutService,  @Inject(LOCAL_STORAGE) private storage: WebStorageService,) {
      ...
      this._BgmratingsComponentService.getAdminCacheWithAIList().subscribe(resp => {
        console.log(resp);
        this.spinner=true;
        this.tableList=resp.dropDownColumnvalues.placeList14.placeList14;
        for(let j=0;j<this.tableList.length;j++)
        {
           ...
           this.cols = [
			...
			{ field: 'equivalent', header: 'Equivalent' },
				   ...
				   this.dboClass.subScopeCd=this.finalFrameList[j].subScopeCd;
				    this.dboClass.condTypeId=this.finalFrameList[j].condTypeId;
					 this.dboClass.panelType=this.finalFrameList[j].panelType;
					  this.dboClass.equipment=this.finalFrameList[j].equipment;
					   this.dboClass.equivalent=this.finalFrameList[j].equivalent;
					    this.dboClass.editFlag=this.finalFrameList[j].editFlag;
						 this.dboClass.activeNew=1;

  this.adminForm.subSupplierMastList.push(this.dboClass);
  }
  this._BgmratingsComponentService.getAdminSubSupplierMast(this.adminForm).subscribe(res => {
    console.log(res);
    this.adminForm = res;
