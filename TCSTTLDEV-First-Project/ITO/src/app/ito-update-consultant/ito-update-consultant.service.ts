...
@Injectable()
export class ITOupdateConsultHomeService{
    private _createConsulttUrl:string="http://10.0.0.4:8082/ITO/custprofile/createConsultant";
    private _updateConsulttUrl:string="http://10.0.0.4:8082/ITO/custprofile/updateConsultant";
   ...
   updateConsultant(consultantDetails:consultantDetails): Observable<any> {
        let headers = new Headers({ 'Content-Type': 'application/json' });
            let options = new RequestOptions({ headers: headers });
            return this._http.post(this._updateConsulttUrl,consultantDetails, options).map(this.extractData);
    }
