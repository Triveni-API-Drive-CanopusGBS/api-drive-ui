...
import { BgmratingsComponentService } from './bgmratings.service';
...
import { dboClass } from './bgmratings';
...
@Component({
  selector: 'app-bgmratings',
  templateUrl: './bgmratings.component.html',
  styleUrls: ['./bgmratings.component.css']
})
export class BgmratingsComponent implements OnInit {
  ...
  constructor(private _ITOturbineConfigService: ITOturbineConfigService, private _ITOScopeOfSupplyService: ITOScopeOfSupplyService,
    ...
    private _ITOeditQoutService: ITOeditQoutService,  @Inject(LOCAL_STORAGE) private storage: WebStorageService,) {
      ...
      this._BgmratingsComponentService.getAdminCacheWithAIList().subscribe(resp => {
        console.log(resp);
this.spinner=true;
        this.tableList=resp.dropDownColumnvalues.placeList4.placeList4;
        for(let j=0;j<this.tableList.length;j++)
        {
           ...
           this.cols = [
            ...
            { field: 'bgmType', header: 'BGM Type' },
            { field: 'bgmRating', header: 'BGM Rating' },
...
update()
{
  ...
  this.dboClass.bgmType=this.finalFrameList[j].bgmType;
  this.dboClass.bgmRating=this.finalFrameList[j].bgmRating;
  ...
  this.adminForm.adminBgmCalcList.push(this.dboClass);
  }
  this._BgmratingsComponentService.getAdminBgmCalc(this.adminForm).subscribe(res => {
    console.log(res);
    this.adminForm = res;
