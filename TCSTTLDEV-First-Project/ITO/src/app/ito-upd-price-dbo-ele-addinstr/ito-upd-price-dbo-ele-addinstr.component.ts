...
@Component({
  selector: 'app-ito-upd-price-dbo-ele-addinstr',
  templateUrl: './ito-upd-price-dbo-ele-addinstr.component.html',
  styleUrls: ['./ito-upd-price-dbo-ele-addinstr.component.css']
})
export class ItoUpdPriceDboEleAddinstrComponent implements OnInit {
  ...
  constructor(private _ItoUpdateCostDBOElecPrice: ItoUpdateCostDBOElecPrice, private _ITOLoginService: ITOLoginService,
    ...
    this._ItoUpdateCostDBOElecPrice.getFrameAndUserData().subscribe(res => {
      ...
      if (this._ITOMyWorkListPageService.selectedUR) {
        ...
        if (this.selectedUR.updateCode == "UPD_DBO_ELE_ADDINSTR_PRICE_NEW") {
          this.exist = true;
          this.tabIndex = 1;
          this.addNew = false;
        } else if (this.selectedUR.updateCode == "UPD_DBO_ELE_ADDINSTR_PRICE") {
          this.addNew = true;
          this.exist = false;
          this.tabIndex = 0
        }
        ...
        if (this.storage.get(this.currentRole) == "DBO_ELE_EDIT") {
          ...
          this.panelCode = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddInstrList[0].panelCode;
          this.itemId = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddInstrList[0].itemId;
          this.itemName = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddInstrList[0].itemName;
          this.custCode = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddInstrList[0].custCode;  // change this after adding in db
          if (this.custCode == "DM") {
          ...
          for (let i = 0; i < this._ITOMyWorkListPageService.responseTemp.unsavedDboEleAddInstrList.length; i++) {
            for (let p = 0; p < this._ITOMyWorkListPageService.responseTemp.savedDboEleAddInstrList.length; p++) {
              if (this._ITOMyWorkListPageService.responseTemp.unsavedDboEleAddInstrList[i].itemId == this._ITOMyWorkListPageService.responseTemp.savedDboEleAddInstrList[p].itemId) {
                this.DBOData.push(this._ITOMyWorkListPageService.responseTemp.unsavedDboEleAddInstrList[i]);
              }
            }
          }
          // get selected data from savedDboEleList
          this.finalRangeList = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddInstrList;
        ...
        // if current role is Reviewer/Approver this else if will be exceuted
          ...
          this.panelCode = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddInstrList[0].panelCode;
          this.panelCdNew = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddInstrList[0].panelCode;
          this.itemId = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddInstrList[0].itemId;
          this.itemName = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddInstrList[0].itemName;
          this.itemNameNew = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddInstrList[0].itemName;
          this.custCode = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddInstrList[0].custCode;  // change this after adding in db
          if (this.custCode == "DM") {
            this.custType = "Domestic";
          } else if (this.custCode == "EX") {
          ...
          this.DBOData = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddInstrList;
          this.finalRangeList = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddInstrList;
          if (this.DBOData.length > 0) {
            this.tableDiv = true;
          }
        ...
        this.cols = [
          ...
          { field: 'addInstrName', header: 'Add Instr Name' },
  ...
  getTableData(val) {
    ...
    this.tempRes.tableName = "ADDITIONAL_INSTRUMENTS";
  ...
  getItemDetails(val) {
    ...
    this._ItoUpdateCostDBOElecPrice.getDBOElectricalData(val).subscribe(res => {
      console.log(res);
      this.tempRes = res;
      this.dBOElectricalListArray=res.generalAddOnList;
        ...
        if (this.DBOData.length != 0) {
          for (let d = 0; d < this.DBOData.length; d++) {
            for (let f = 0; f < this.finalRangeList.length; f++) {
              if (this.DBOData[d].addInstrCd == this.finalRangeList[f].addInstrCd) {
                document.getElementById(this.DBOData[d].addInstrCd).style.backgroundColor = "#0275d8";
  ...
  save() {
    ...
    if (this.finalRangeList.length != 0) {
      for (let s = 0; s < this.finalRangeList.length; s++) {
        if (this.finalRangeList[s].addInstrCd === this.selectedRange.addInstrCd) {
          this.finalRangeList[s] = this.selectedRange;
          this.contains = true;
          let butn = document.getElementById(this.finalRangeList[s].addInstrCd).style.backgroundColor = "#0275d8";
          break;
        }
      }
      if (!this.contains) {
  ...
  SaveAsDraft() {
    ...
    for (let q = 0; q < this.finalRangeList.length; q++) {
     // if (this.finalRangeList[q].addInstrCd > 0) {
        this.saveBasic.updateCode = "UPD_DBO_ELE_ADDINSTR_PRICE";
    //  } else {
       // this.saveBasic.updateCode = "UPD_DBO_ELE_ADDINSTR_PRICE_NEW";
    //  }
    }
    ...
    this._ItoUpdDboEleSpladdonService.createDboEleUpdateRequestPriceAddInstr(this.tempRes1).subscribe(resp => {
      console.log(resp);
