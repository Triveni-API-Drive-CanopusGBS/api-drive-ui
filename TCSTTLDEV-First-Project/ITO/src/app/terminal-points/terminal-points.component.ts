...
@Component({
  selector: 'app-terminal-points',
  templateUrl: './terminal-points.component.html',
  styleUrls: ['./terminal-points.component.css']
})
export class TerminalPointsComponent implements OnInit {

  dboFormDataaa: any; //To store dbo form response
  quotId:number=0;
  terminal: string = 'terminal'; // local storage value
  terminalLocal: Array<any> = [];
  display: Array<any> = [];
  dboClass: dboClass;
  othetList: Array<any> = [];
  symbol:string="*";
 ...
 disablerhterminal:Array<boolean>=[];
 ...
 mainSave:boolean=true;
  ...
  constructor(private _ItoPerformanceService: ItoPerformanceService,private _ITOLoginService: ITOLoginService, private _ITOeditQoutService: ITOeditQoutService,  private _ITOcustomerRequirementService: ITOcustomerRequirementService,
    ...
    this.terminalLocal[this.terminal] = this.storage.get(this.terminal);
    if(this._ITOcustomerRequirementService.editFlag==true && this._ITOeditQoutService.terminalpo!=undefined)
{

  this.tempArray=this._ITOeditQoutService.terminalpo;
  this.mainSave=false;
  ...
  for(let x=0;x<this.tempArray.length;x++)
  {
    ...
    if (this.storage.get(this.terminal) == null) {
      this.saveInLocal(this.terminal, {
        othetList:this.othetList,itemOthersList1:this.itemOthersList1,arraycheck:this.arraycheck
      });
    }
    if(this.storage.get(this.terminal).othetList.length!=0){
      this.othetList= this.storage.get(this.terminal).othetList;
      ...
      this.arraycheck= this.storage.get(this.terminal).arraycheck;
      this.mainSave=false;

   }
   if(this.storage.get(this.terminal).itemOthersList1.length!=0){
    this.itemOthersList1= this.storage.get(this.terminal).itemOthersList1;
    ...
    if(this.othetList.length==0)
    {
        ...
        console.log(this.storage.get(this.terminal));
        this._ItoPerformanceService.getOtherChapter(this.dboFormDataaa).subscribe(res => {
         ...
         {
          if(res.otherDataList[x].scopeCd=="TP")
          {
...
if(this.tempArray[y].seqNo==this.othetList[x].seqNo && this.tempArray[y].itemId==this.othetList[x].itemId )
{
  ...
  saveInLocal(key, val): void {
    ...
    this.terminalLocal[key] = this.storage.get(key);
  }
  saveOthers()
  {

    this.getPrice=true;
    if(this.disablerhterminal.includes(true) )
    {
      this.getPrice=false;
    }
    if(this.getPrice )
  ...
  compOthersForm1(others, othersfrm: NgForm,id,name) {
    ...
    this.dboClass=new dboClass();
      ...
      this.dboClass.remarks=null;
      this.itemOthersList1.push(this.dboClass);
     // this.disablerhterminal[this.itemOthersList1.length-1]=false;
    console.log(others);
    console.log(name);
  ...
  cancelLinesOth1(i,id) {
    ...
    this.disablerhterminal.splice(i,1);
...
saveterminal(i)
{
  ...
  this.disablerhterminal[i]=false;
