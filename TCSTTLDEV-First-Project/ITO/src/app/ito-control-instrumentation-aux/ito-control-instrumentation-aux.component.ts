...
@Component({
  selector: 'app-ito-control-instrumentation-aux',
  templateUrl: './ito-control-instrumentation-aux.component.html',
  styleUrls: ['./ito-control-instrumentation-aux.component.css']
})
// export class ItoControlInstrumentationAuxComponent implements OnInit {
...
export class ItoControlInstrumentationAuxComponent implements OnInit, AfterContentChecked {
  ...
  dboEleCIAuxFullArray: Array<any> = []; //store complete data from getEleTechPrice
...
childvaluestemp1:Array<any>=[]; // to temporary store child values.
 ...
 dboEleCIAuxFull: string = 'dboEleCIAuxFull'; // ; local storage name for electrical auxilaries.
...
constructor(private _DboEleAuxialriesService: DboEleAuxialriesService, private _ITOLoginService: ITOLoginService,
 ...
 if(currentRole == "QUOT_REVIWER" || currentRole == "QUOT_APPROVER"){
    ...
    //To set to null in the beginning of the edit mode
       ...
       //Multifunctional digital generator protection fvrelay shall have following protections features
      // this._ITOeditQoutService.dboEleCIAuxData
       console.log( this._ITOeditQoutService.dboEleCIAuxData);
       if(this._ITOeditQoutService.dboEleCIAuxData.length >0){
         this.finalCostBool = true;
         this.mainSave2=false;
  this.mainSave=false;
       }
       console.log(this._ITOeditQoutService.dboEleItmOthers);
       if( this._ITOcustomerRequirementService.editFlag==true && this._ITOeditQoutService.dboEleCIAuxData!=undefined)
       {
        this.temparray1= this._ITOeditQoutService.dboEleCIAuxData;
		    //  if(this._ITOeditQoutService.dboEleCIAuxData.length>0){
        // this.finalEleAuxCost=this._ITOeditQoutService.dboEleCIAuxData[0].itemCost;
        // }
       }
    ...
    this._ITOturbineConfigService.getDboFormData().subscribe(ress => {
      ...
      this.dboElectricalLocal[this.dboEleCIAuxFull] = this.storage.get(this.dboEleCIAuxFull);
      console.log(this.storage.get(this.dboEleCIAuxFull));
      if (this.storage.get(this.dboEleCIAuxFull) == null) {
      this.saveInLocal(this.dboEleCIAuxFull, {
        dboEleCIAuxFullArray: this.dboEleCIAuxFullArray, addOnList: this.addOnList, itemOthersList: this.itemOthersList,
      ...
      this.saveSubCost= this.storage.get(this.dboEleCIAuxFull).saveSubCost;
    
   
    if(this.storage.get(this.dboEleCIAuxFull).dboEleCIAuxFullArray.length!=0){
      this.dboEleCIAuxFullArray= this.storage.get(this.dboEleCIAuxFull).dboEleCIAuxFullArray;
    this.finalCostBool = true;
    this.mainSave2=false;
  this.mainSave=false;
   }
   if(this.storage.get(this.dboEleCIAuxFull).addOnList.length!=0){
    this.addOnList= this.storage.get(this.dboEleCIAuxFull).addOnList;
  
 }
 if(this.storage.get(this.dboEleCIAuxFull).itemIdList.length!=0){
  this.itemIdList= this.storage.get(this.dboEleCIAuxFull).itemIdList;

}
if(this.storage.get(this.dboEleCIAuxFull).itemIdList1.length!=0){
this.itemIdList1= this.storage.get(this.dboEleCIAuxFull).itemIdList1;

}
if(this.storage.get(this.dboEleCIAuxFull).selectdEl!=undefined){
this.selectdEl= this.storage.get(this.dboEleCIAuxFull).selectdEl;

}
//To get selectedEl1 from local storage
if(this.storage.get(this.dboEleCIAuxFull).selectdEl1!=undefined){
this.selectdEl1= this.storage.get(this.dboEleCIAuxFull).selectdEl1;
}
//to display new item creation other from local storage
if(this.storage.get(this.dboEleCIAuxFull).itemOthersAddonList.length != 0){
  this.itemOthersAddonList = this.storage.get(this.dboEleCIAuxFull).itemOthersAddonList;
}
 // to dispaly new item creation other with lhs/rhs from local storage
 if(this.storage.get(this.dboEleCIAuxFull).lhsRhsItemsList.length != 0){
  this.lhsRhsItemsList = this.storage.get(this.dboEleCIAuxFull).lhsRhsItemsList;
}
 if(this.storage.get(this.dboEleCIAuxFull).itemRemarksVal!=null  && this.storage.get(this.dboEleCIAuxFull).itemRemarksVal != ""){
  this.itemRemarksVal= this.storage.get(this.dboEleCIAuxFull).itemRemarksVal;
  this.othItmTechRemChk=true;
 this.itemtechRemarkCheck=true;
 this.itemRemarkDiv=true;

}
if(this.storage.get(this.dboEleCIAuxFull).itemCmrRemarksVal!=null  && this.storage.get(this.dboEleCIAuxFull).itemCmrRemarksVal != ""){
this.itemCmrRemarksVal= this.storage.get(this.dboEleCIAuxFull).itemCmrRemarksVal;
this.othItmComrRemChk=true;
this.itemComrRemarkCheck=true;
this.itemComrRemarkDiv=true;

}
if(this.storage.get(this.dboEleCIAuxFull).subItemRmkValOut!=null  && this.storage.get(this.dboEleCIAuxFull).subItemRmkValOut != ""){
this.subItemRmkValOut= this.storage.get(this.dboEleCIAuxFull).subItemRmkValOut;
this.othSubItmTechRemChk=true;
this.subItemTchRem=true;
this.subItemRemarkDiv=true;

}
if(this.storage.get(this.dboEleCIAuxFull).subItemCmrRemValOut!=null && this.storage.get(this.dboEleCIAuxFull).subItemCmrRemValOut != ""){
this.subItemCmrRemValOut= this.storage.get(this.dboEleCIAuxFull).subItemCmrRemValOut;
this.othSubItmComrRemChk=true;
this.subItemComRem=true;
this.subItemComrRemDiv=true;
}
if(this.storage.get(this.dboEleCIAuxFull).subItemOthAddonList.length!=0 ){
  this.lhsRhsSubItemsList= this.storage.get(this.dboEleCIAuxFull).lhsRhsSubItemsList;
    this.subItemOthAddonList=this.storage.get(this.dboEleCIAuxFull).subItemOthAddonList;
  }
  if(this.storage.get(this.dboEleCIAuxFull).dessubItemOthAddonList.length!=0 ){
    this.lhsRhsDesSubItemsList= this.storage.get(this.dboEleCIAuxFull).lhsRhsDesSubItemsList;
    this.dessubItemOthAddonList=this.storage.get(this.dboEleCIAuxFull).dessubItemOthAddonList;
...
if( this._ITOcustomerRequirementService.editFlag==true && this._ITOeditQoutService.checkEdit == false){
  if(this.oneLineLocArray != null){
	  //  if(this.oneLineLocArray.cIAuxCost !=0){
  this.finalEleAuxCost = this.oneLineLocArray.cIAuxCost;  
	  //  }
  }
  }else{
    // if(this.storage.get(this.dboEleCIAuxFull).finalEleAuxCost > 0){
    this.finalEleAuxCost = this.storage.get(this.dboEleCIAuxFull).finalEleAuxCost;
  // }
}
console.log(this.dboFormData);
    this._DboEleAuxialriesService.getEleItems(this.dboFormData).subscribe(respon => {
      ...
      for(let o=0; o<respon.dboEleItemList.length; o++){
        if(respon.dboEleItemList[o].eleType == "CI_AUX"){
...
this.defaulVales1=[];
 ...
 this.Ifapplicabletemp=[];
           ...
           //pushing selected components data fromm storage to local variable
              this.dboEleCIAuxFullArray.push({
                counter:0,
              ...
              console.log(this.dboEleCIAuxFullArray);
      ...
      if(this._ITOcustomerRequirementService.editFlag==true)
      {
          ...
          if(this.dboEleCIAuxFullArray[0].techComments!=null)
          {
            this.itemRemarksVal= this.dboEleCIAuxFullArray[0].techComments;
            this.othItmTechRemChk=true;
           this.itemtechRemarkCheck=true;
           this.itemRemarkDiv=true;
          }
          if(this.dboEleCIAuxFullArray[0].comrComments!=null)
          {
            this.itemCmrRemarksVal= this.dboEleCIAuxFullArray[0].comrComments;
            this.othItmComrRemChk=true;
            this.itemComrRemarkCheck=true;
            this.itemComrRemarkDiv=true;
          }
          if(this.dboEleCIAuxFullArray[0].techRemarks!=null)
          {
            this.subItemRmkValOut= this.dboEleCIAuxFullArray[0].techRemarks;
            this.othSubItmTechRemChk=true;
            this.subItemTchRem=true;
            this.subItemRemarkDiv=true;
          }
          if(this.dboEleCIAuxFullArray[0].comrRemarks!=null)
          {
            this.subItemCmrRemValOut= this.dboEleCIAuxFullArray[0].comrRemarks;
            this.othSubItmComrRemChk=true;
            this.subItemComRem=true;
            this.subItemComrRemDiv=true;
          }       
...
// take min and max value
  ...
  getFilterData()
  {
    ...
    // ItemApplInd
    // ModifiedById
    if(this.dboEleCIAuxFullArray.length>0)
    {
    for( let j=0;j<this.dboEleCIAuxFullArray.length;j++)
    {
      if(this.dboEleCIAuxFullArray[j].itemId==this.itemId && this.dboEleCIAuxFullArray[j].subItemId==this.subItemId)
      {
        this.dboEleCIAuxFullArray.splice(j,1);
      }
    }
  }
...
this.rhsdes[k]=false;
  ...
  this.newArryUpd=[];
          enable=false;
          this.enablebutton=enable;
         this.defaulVales1=this.dboEleCIAuxFullArray[j].defaulVales1;
        for(let j=0;j<res.dboEleSpecialFilterList.length;j++)
        {
...
this.defaultValues=[];
this.newPanelval=[];
this.newPanelval=this.dboEleCIAuxFullArray[j].newPanelval1;

  for(let i=0;i<this.questionsBean.length;i++)
  {
    let countertemp=0;
    for(let k=0;k<this.dboEleCIAuxFullArray[j].defaultValuesid.length;k++)
    {
      if(this.dboEleCIAuxFullArray[j].defaultValuesid[k]==this.questionsBean[i].orderId || this.questionsBean[i].colType=="CHECKBOX" || this.questionsBean[i].colType=="DROPDOWNCHECKBOX" )
      {
  ...
  this.techFlagindicator=[];
   this.techFlagindicator=this.dboEleCIAuxFullArray[j].tempflagsave;
  
  this.inptCstQty=[];
  this.colNumber=[];
  this.colNumber=this.dboEleCIAuxFullArray[j].colNumber;
  this.costQty=[];
  this.inptCstQty=this.dboEleCIAuxFullArray[j].inptCstQty;
  this.costQty=this.dboEleCIAuxFullArray[j].costQty;
  this.inputText=[];
  this.inputText=this.dboEleCIAuxFullArray[j].inputText;
  this.lhsCheck=[];
  this.lhsNmCheck==[];
  this.lhsCheck= this.dboEleCIAuxFullArray[j].lhsCheck;
  this.lhsNmCheck= this.dboEleCIAuxFullArray[j].lhsNmCheck;
  this.drpCheck=[];
  this.dropCheckQty=[];
   this.drpCheckqtyy=[];
   this.drpCheckqtyyKey=[];
  // this.drpCheck=this.dboEleCIAuxFullArray[j].drpCheck;
   //this.dropCheckQty=this.dboEleCIAuxFullArray[j].dropCheckQty;
    //this.drpCheckqtyy=this.dboEleCIAuxFullArray[j].drpCheckqtyy;
    //this.drpCheckqtyyKey=this.dboEleCIAuxFullArray[j].drpCheckqtyyKey;
    for(let ji=0;ji<this.dboEleCIAuxFullArray[j].drpCheckqtyyKey.length;ji++)
    {
      if(this.dboEleCIAuxFullArray[j].drpCheckqtyyKey[ji]!=null)
      {
        this.drpCheckqtyyKey[ji]=this.dboEleCIAuxFullArray[j].drpCheckqtyyKey[ji];
      }
    }
    ...
    {
	if(this._ITOcustomerRequirementService.editFlag==true && this.dboEleCIAuxFullArray[j].counter==0)
{
   for(let value1 in this.drpCheckqtyyKey)
   {
     let value2=Number(value1);
 for(let value in this.dboEleCIAuxFullArray[j].drpCheckqtyy)
 {
   ...
   for(let k=0;k<this.questionsBean[value2-1].dropDownValueList.length;k++)
   {
    ...
    if(value==this.questionsBean[value2-1].dropDownValueList[k].value)
    {
       ...
       this.drpCheckqtyy[k]=this.dboEleCIAuxFullArray[j].drpCheckqtyy[value];
    }
   }
 }
   }
  ...
  else
{
   ...
   this.drpCheckqtyyKey=this.dboEleCIAuxFullArray[j].drpCheckqtyyKey;
    ...
    this.errMsgGreen = [];
       ...
       this.lhsNmCheck==[];
        ...
        this.newPanelval=[];
        if(this.dboEleCIAuxFullArray[j].panelapplicable==false )
        {
          this.saveBtColor=false;
          this.buttonColor="red";

        }
        else if(this.dboEleCIAuxFullArray[j].panelapplicable==true && this.dboEleCIAuxFullArray[j].panelapplicablescm==true )
        {
          this.saveBtColor=false;
        ...
        else if(this.dboEleCIAuxFullArray[j].panelapplicablescm==false )
        {
          this.saveBtColor=false;
          this.buttonColor="blue";
        }
				  this.Ifapplicable=[];
				          this.Ifapplicable=this.dboEleCIAuxFullArray[j].Ifapplicable;
        // this.drpCheck=this.dboEleCIAuxFullArray[j].drpCheck;
        // this.dropCheckQty=this.dboEleCIAuxFullArray[j].dropCheckQty;
        //  this.drpCheckqtyy=this.dboEleCIAuxFullArray[j].drpCheckqtyy;
        //  this.drpCheckqtyyKey=this.dboEleCIAuxFullArray[j].drpCheckqtyyKey;
      this.itemcost=this.dboEleCIAuxFullArray[j].itemcost;
       this.lhsCheck= this.dboEleCIAuxFullArray[j].lhsCheck;
       this.lhsNmCheck= this.dboEleCIAuxFullArray[j].lhsNmCheck;
       this.errMsgRhsCost=this.dboEleCIAuxFullArray[j].errMsgRhsCost;
       this.errMsgRed=this.dboEleCIAuxFullArray[j].errMsgRed;
       this.errMsgBlue=this.dboEleCIAuxFullArray[j].errMsgBlue;
       this.errMsgGreen=this.dboEleCIAuxFullArray[j].errMsgGreen;
       this.errMsgPnl=this.dboEleCIAuxFullArray[j].errMsgPnl;
       this.errDisplayPnl=this.dboEleCIAuxFullArray[j].errDisplayPnl;
       this.newPanelval=this.dboEleCIAuxFullArray[j].newPanelval1;;
 //this.defaultValuesid=this.dboEleCIAuxFullArray[j].errDisplayPnl1;;
 
        if(this.dboEleCIAuxFullArray[j].eleItemTechRemarks!=null)
        {
          this.eleItemTechRemarks=this.dboEleCIAuxFullArray[j].eleItemTechRemarks;
          this.techCheckIn = true;
          this.itemTechRmkDiv=true;
        }
        if(this.dboEleCIAuxFullArray[j].eleItemComrRemarks!=null)
        {
          this.eleItemComrRemarks=this.dboEleCIAuxFullArray[j].eleItemComrRemarks;
          this.comrrCheck = true;
          this.itemComrRmkDiv=true;
        }
        this.defaultValues=this.dboEleCIAuxFullArray[j].defaultValues;
        this.qty=this.dboEleCIAuxFullArray[j].qty;
        this.dboCost=this.dboEleCIAuxFullArray[j].dboCost;
        this.discountPer = this.dboEleCIAuxFullArray[j].discountPer;
        if(this.dboEleCIAuxFullArray[j].panelapplicable==false )
        {
          this.saveBtColor=false;
          this.buttonColor="red";
        ...
        this.dboAddOnCost = this.dboEleCIAuxFullArray[j].dboAddOnCost;
        this.othCheck=[];
        this.displayCompOthTable=[];
        this.othersCompCheck=[];

      if(this.dboEleCIAuxFullArray[j].itemOthersList.length!=0)
      {
        for(let i=0;i<this.dboEleCIAuxFullArray[j].itemOthersList.length;i++)
        {
          if(this.dboEleCIAuxFullArray[j].itemOthersList[i].itemId==this.itemId && this.dboEleCIAuxFullArray[j].itemOthersList[i].subItemId==this.subItemId )
          {            
           // this.displayCompOthTable=true;
         
            //this.othersCompCheck=true;
            this.itemOthersList.push(this.dboEleCIAuxFullArray[j].itemOthersList[i]);
  ...
  this.defaultValues=[];
  this.defaultValues=this.dboEleCIAuxFullArray[j].defaultValues;
  if(this._ITOcustomerRequirementService.editFlag==true && this.dboEleCIAuxFullArray[j].counter==0 &&  this._ITOeditQoutService.dboEleCIAuxData.length!=0)
 { 
  this.defaultValues=[];
  this.defaultValues1=[];
  for(let j=0;j<300;j++)
 ...
 for(let j=0;j<this.questionsBean.length;j++)
 {
  ...
  for(let j=0;j< this._ITOeditQoutService.dboEleCIAuxData.length;j++)
  {
    if( this._ITOeditQoutService.dboEleCIAuxData[j].itemId==this.itemId &&  this._ITOeditQoutService.dboEleCIAuxData[j].dispInd==1 &&  this._ITOeditQoutService.dboEleCIAuxData[j].addOnNewColFlag ==0 &&   this._ITOeditQoutService.dboEleCIAuxData[j].lhsFlag ==0)
    {
      this.defaultValues1[ this._ITOeditQoutService.dboEleCIAuxData[j].orderId-1]= this._ITOeditQoutService.dboEleCIAuxData[j].colValCd;
    }
  }
  for(let j=0;j<this.defaultValues1.length;j++)
  {
  ...
  console.log(this.dboEleCIAuxFullArray[j].defaultValues);
  this.defaultValuesid=[];
  for(let k = 0; k < this.questionsBean.length; k++){
    if(this.questionsBean[k].dispInd==1)
    {
  ...
  this.temparray=this.dboEleCIAuxFullArray[j].temparray;
  for(let j=0;j<this.childvaluesnew.length;j++)
  {
  ...
  this._DboEleAuxialriesService.getEleTechPrice(this.dboFormData).subscribe(res => {
   ...
   // this.Ifapplicabletemp = this.Ifapplicabletemp.filter(n => n != null);
    ...
    if (res.successCode == 0) {
      ...
      // this._ITOeditQoutService.dboEleCIAuxData = res.eleTechList;
    ...
    if (this.dboEleCIAuxFullArray.length != 0) {
      for (let d = 0; d < this.dboEleCIAuxFullArray.length; d++) {
        let j = this.dboEleCIAuxFullArray.map(d => { return d.id }).indexOf(this.eleItemId);
        if (j != (-1)) {
          this.dboEleCIAuxFullArray[j] = {
            counter:1,
            qty: this.qty,
    ...
    console.log(this.dboEleCIAuxFullArray);
    this.addedClassList = [];
    for (let m = 0; m < this.dboEleCIAuxFullArray.length; m++) {
      this.addedClassList.push(this.dboEleCIAuxFullArray[m].componenet);
    }
...
calcTotal()
{
 ...
 // this.dboFormData.modBy = this._ITOLoginService.loggedUserDetails;
  this.dboFormData.modifiedById = this._ITOLoginService.loggedUserDetails; 
  this.dboFormData.quotId = this._ITOcustomerRequirementService.saveBasicDet.quotId;
  this.dboFormData.scopeCd = "CI_AUX";
  ...
  if(this.storage.get(this.dboEleCIFull)!=null)
  {
    if(this.storage.get(this.dboEleCIFull).itemIdList.length!=0){
      this.itemIdListAux= this.storage.get(this.dboEleCIFull).itemIdList;
 ...
 this.dboFormData.modifiedById = this._ITOLoginService.loggedUserDetails; 
  ...
  this._ITOturbineConfigService.getExcelCostSheetData(this._ITOcustomerRequirementService.saveBasicDet.quotId).subscribe(resOnline => {
    ...
    if (this._ITOcustomerRequirementService.editFlag && this._ITOeditQoutService.checkEdit==false ) {
      ...
      this.router.navigate(['/CostEstimation/ControlInstrumentation/DboEleCiextdScope']);
    }
  });
  this.hideprogressCost1 = false;
  });
...
itemComrCheckIn(event){
  ...
  dboEleItmList(event, itemId, i,subitemid)
  {
    ...
    if (!event.target.checked) { // on uncheck 
      ...
      console.log(this.dboEleCIAuxFullArray);
      for(let k = 0; k < this.dboEleCIAuxFullArray.length; k++){
        if(this.dboEleCIAuxFullArray[k].itemId == itemId){
            this.dboEleCIAuxFullArray[k] = null;
      ...
      this.dboEleCIAuxFullArray = this.dboEleCIAuxFullArray.filter(n => n != null);
  ...
  dboEleItmListSub(event, itemId, i){
    ...
    if (!event.target.checked) { // on uncheck 
      ...
      for(let k=0; k<this.dboEleCIAuxFullArray.length; k++){
        if(this.dboEleCIAuxFullArray[k].subItemId == itemId){
          this.dboEleCIAuxFullArray[k] = null;
          this.dboEleCIAuxFullArray.splice(k,1);
          break;
        }
      }
      // for(let k = 0; k < this.tempitemOthersList.length; k++){
      ...
      for( let j=0;j<this.dboEleCIAuxFullArray.length;j++)
      {
        if(this.dboEleCIAuxFullArray[j].itemId==this.itemId && this.dboEleCIAuxFullArray[j].subItemId==this.subItemId)
        {
          ...
          if(this.dboEleCIAuxFullArray[j].itemOthersList.length!=0)
          {
            for(let i=0;i<this.dboEleCIAuxFullArray[j].itemOthersList.length;i++)
            {
              if(this.dboEleCIAuxFullArray[j].itemOthersList[i].itemId==this.itemId && this.dboEleCIAuxFullArray[j].itemOthersList[i].subItemId==this.subItemId && this.dboEleCIAuxFullArray[j].itemOthersList[i].colNm==name  && this.dboEleCIAuxFullArray[j].itemOthersList[i].colValCd==value )
              {
                this.dboEleCIAuxFullArray[j].itemOthersList[i]=null;
                this.dboEleCIAuxFullArray[j].itemOthersList.splice(i,1);
              }
            }
          }
        }
   ...
   // this.dboFormData.modBy = this._ITOLoginService.loggedUserDetails;
    ...
    if(this.storage.get(this.dboEleCIFull)!=null)
    {
      if(this.storage.get(this.dboEleCIFull).itemIdList1.length!=0){
        this.itemIdListAux= this.storage.get(this.dboEleCIFull).itemIdList1;
      for(let j=0;j<this.itemIdListAux.length;j++)
      {
        if(this.itemIdListAux[j]!=null)
        {
