...
@Component({
  selector: 'app-admin-spares',
  templateUrl: './admin-spares.component.html',
  styleUrls: ['./admin-spares.component.css']
})
export class AdminSparesComponent implements OnInit {
  ...
  constructor(private _ITOturbineConfigService: ITOturbineConfigService, private _ITOScopeOfSupplyService: ITOScopeOfSupplyService,
    ...
    private _ITOeditQoutService: ITOeditQoutService,  @Inject(LOCAL_STORAGE) private storage: WebStorageService,) {
      ...
      this._BgmratingsComponentService.getAdminCacheWithAIList().subscribe(resp => {
        console.log(resp);
        this.spinner=true;
        this.tableList=resp.dropDownColumnvalues.placeList13.placeList13;
        for(let j=0;j<this.tableList.length;j++)
        {
           ...
           this.cols = [
            ...
            { field: 'custType', header: 'CustType' },
				   ...
				   this.dboClass.frmId=this.finalFrameList[j].frmId;
				    this.dboClass.custType=this.finalFrameList[j].custType;
					 this.dboClass.description=this.finalFrameList[j].description;
					  this.dboClass.qty=this.finalFrameList[j].qty;
					    this.dboClass.editFlag=this.finalFrameList[j].editFlag;
						 this.dboClass.activeNew=1;

  this.adminForm.sparesMastList.push(this.dboClass);
  }
  this._BgmratingsComponentService.getAdminSparesMast(this.adminForm).subscribe(res => {
    console.log(res);
    this.adminForm = res;
