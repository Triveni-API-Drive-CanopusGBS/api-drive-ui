...
@Component({
  selector: 'app-techinal',
  templateUrl: './techinal.component.html',
  styleUrls: ['./techinal.component.css']
})
export class TechinalComponent implements OnInit {
  ...
  constructor(private _Router: Router, private _ITOcustomerRequirementService: ITOcustomerRequirementService,
    ...
    //this._ITOcustomerRequirementService.sendbtnStatus(true, false, true, true, true, false, false);
    ...
    if (this.estimateCostLocal[this.scopeofsupp] != null) {
      ...
      if (this.selectedScopes.length != 0) {
        for (let s = 0; s < this.selectedScopes.length; s++) {
          console.log(this.selectedScopes[s].scopeCode, this.selectedScopes[s].scopeName);
