...
@Component({
  selector: 'app-admin-terminal-mast',
  templateUrl: './admin-terminal-mast.component.html',
  styleUrls: ['./admin-terminal-mast.component.css']
})
export class AdminTerminalMastComponent implements OnInit {
  ...
  constructor(private _ITOturbineConfigService: ITOturbineConfigService, private _ITOScopeOfSupplyService: ITOScopeOfSupplyService,
    ...
    private _ITOeditQoutService: ITOeditQoutService,  @Inject(LOCAL_STORAGE) private storage: WebStorageService,) {
      ...
      this._BgmratingsComponentService.getAdminCacheWithAIList().subscribe(resp => {
        console.log(resp);
        this.spinner=true;
        this.tableList=resp.dropDownColumnvalues.placeList16.placeList16;
        for(let j=0;j<this.tableList.length;j++)
        {
           ...
           this.cols = [
            ...
            {field: 'subScopeCd', header: 'Sub Scope Code'},
            { field: 'condTypeName', header: 'Cond Type' },
            { field: 'panelType', header: 'Panel Type' },
            { field: 'bleedTypeName', header: 'Bleed Type'},
            {field: 'variantTypeName', header: 'Variant Type'},
...
update()
{
 ...
 this.dboClass.id=this.finalFrameList[j].id;
				  this.dboClass.itemId=this.finalFrameList[j].itemId;
				   this.dboClass.subScopeCd=this.finalFrameList[j].subScopeCd;
				    this.dboClass.condTypeId=this.finalFrameList[j].condTypeId;
           this.dboClass.panelType=this.finalFrameList[j].panelType;
           this.dboClass.bleedTypeId = this.finalFrameList[j].bleedTypeId;
					  this.dboClass.variantType=this.finalFrameList[j].variantType;
					   this.dboClass.description=this.finalFrameList[j].description;
					    this.dboClass.editFlag=this.finalFrameList[j].editFlag;
						 this.dboClass.activeNew=1;

  this.adminForm.adminTerminalMastList.push(this.dboClass);
  }
  this._BgmratingsComponentService.getAdminTerminalMast(this.adminForm).subscribe(res => {
    console.log(res);
    this.adminForm = res;
