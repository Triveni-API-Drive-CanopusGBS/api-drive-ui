...
@Component({
  selector: 'app-admin-perf-ac-dc-mast',
  templateUrl: './admin-perf-ac-dc-mast.component.html',
  styleUrls: ['./admin-perf-ac-dc-mast.component.css']
})
export class AdminPerfAcDcMastComponent implements OnInit {
  ...
  constructor(private _ITOturbineConfigService: ITOturbineConfigService, private _ITOScopeOfSupplyService: ITOScopeOfSupplyService,
    ...
    private _ITOeditQoutService: ITOeditQoutService,  @Inject(LOCAL_STORAGE) private storage: WebStorageService,) {
      ...
      this._BgmratingsComponentService.getAdminCacheWithAIList().subscribe(resp => {
        console.log(resp);
        this.spinner=true;
        this.tableList=resp.dropDownColumnvalues.placeList10.placeList10;
...
update()
{
  ...
  this.dboClass.category=this.finalFrameList[j].category;
  this.dboClass.condTypeId=this.finalFrameList[j].condTypeId;//error
  this.dboClass.itemType=this.finalFrameList[j].itemType;//error


  this.dboClass.itemCd=this.finalFrameList[j].itemCd;//error
  this.dboClass.speed=this.finalFrameList[j].speed;//error
  this.dboClass.voltage=this.finalFrameList[j].voltage;//error
  this.dboClass.feeder=this.finalFrameList[j].feeder;//error
  ...
  this.dboClass.panelType=this.finalFrameList[j].panelType;//error
  this.dboClass.activeNew=1;//error
  ...
  this.adminForm.adminPerfAcDcMastList.push(this.dboClass);
  }
  this._BgmratingsComponentService.getAdminPerfAcDcMast(this.adminForm).subscribe(res => {
    console.log(res);
    this.adminForm = res;
