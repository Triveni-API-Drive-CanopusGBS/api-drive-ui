...
@Component({
  selector: 'app-estimate-cost',
  templateUrl: './estimate-cost.component.html',
  styleUrls: ['./estimate-cost.component.css']
})
export class EstimateCostComponent implements OnInit {

  lubeAndControl: any;
  highSpeedCoupling: any;
  motorised: any;
  stepDownGearBox: any;
  steamTurbineModel: any;
  ...
  constructor(private _Router: Router, private _ITOcustomerRequirementService: ITOcustomerRequirementService,
   ...
   // this._ITOcustomerRequirementService.sendbtnStatus(true, false, true, true, true, true, true, true);
    ...
    if (this.estimateCostLocal[this.scopeofsupp] != null) {
      ...
      if (this.selectedScopes.length != 0) {
        for (let s = 0; s < this.selectedScopes.length; s++) {
          ...
          console.log(this.selectedScopes[s].scopeCode, this.selectedScopes[s].grpCd);
        }
      }
      console.log(this.disableArray);
      console.log(this.disbaleBoolean);
    }
  }
