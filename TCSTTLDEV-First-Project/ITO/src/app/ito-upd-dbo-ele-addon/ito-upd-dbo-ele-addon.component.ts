...
@Component({
  selector: 'app-ito-upd-dbo-ele-addon',
  templateUrl: './ito-upd-dbo-ele-addon.component.html',
  styleUrls: ['./ito-upd-dbo-ele-addon.component.css']
})
export class ItoUpdDboEleAddonComponent implements OnInit {
  ...
  tempdBOElectricalListArray:Array<any>;
  ...
  constructor(private _ItoUpdateCostDBOElecPrice: ItoUpdateCostDBOElecPrice, private _ITOLoginService: ITOLoginService,
    ...
    private _ItoUpdDboEleAddonService:ItoUpdDboEleSpladdonService,
    ...
    this._ItoUpdateCostDBOElecPrice.getFrameAndUserData().subscribe(res => {
      ...
      if (this._ITOMyWorkListPageService.selectedUR) {
        ...
        if (this.selectedUR.updateCode == "UPD_DBO_ELE_ADDON_PRICE_NEW") {
          this.exist = true;
          this.tabIndex = 1;
          this.addNew = false;
        } else if (this.selectedUR.updateCode == "UPD_DBO_ELE_ADDON_PRICE") {
          this.addNew = true;
          this.exist = false;
          this.tabIndex = 0
        }
        ...
        if (this.storage.get(this.currentRole) == "DBO_ELE_EDIT") {
          ...
          this.panelCode = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddOnList[0].panelCode;
          this.itemId = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddOnList[0].itemId;
          this.itemName = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddOnList[0].itemName;
          this.custCode = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddOnList[0].custCode;  // change this after adding in db
          if (this.custCode == "DM") {
          ...
          for (let i = 0; i < this._ITOMyWorkListPageService.responseTemp.unsavedDboEleAddOnList.length; i++) {
            for (let p = 0; p < this._ITOMyWorkListPageService.responseTemp.savedDboEleAddOnList.length; p++) {
              if (this._ITOMyWorkListPageService.responseTemp.unsavedDboEleAddOnList[i].itemId == this._ITOMyWorkListPageService.responseTemp.savedDboEleAddOnList[p].itemId) {
                this.DBOData.push(this._ITOMyWorkListPageService.responseTemp.unsavedDboEleAddOnList[i]);
              }
            }
          }
          // get selected data from savedDboEleList
          this.finalRangeList = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddOnList;
        ...
        // if current role is Reviewer/Approver this else if will be exceuted
          ...
          this.panelCode = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddOnList[0].panelCode;
          this.panelCdNew = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddOnList[0].panelCode;
          this.itemId = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddOnList[0].itemId;
          this.itemName = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddOnList[0].itemName;
          this.itemNameNew = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddOnList[0].itemName;
          this.custCode = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddOnList[0].custCode;  // change this after adding in db
          if (this.custCode == "DM") {
            this.custType = "Domestic";
          } else if (this.custCode == "EX") {
          ...
          this.DBOData = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddOnList;
          this.finalRangeList = this._ITOMyWorkListPageService.responseTemp.savedDboEleAddOnList;
          if (this.DBOData.length > 0) {
            this.tableDiv = true;
          }
        ...
        this.cols = [
          ...
          { field: 'colValCd', header: 'ColValCode' },
        ...
        this.cols = [
          { field: 'itemName', header: 'Item Name' },
          { field: 'minVal', header: 'Min Value' },
           { field: 'maxVal', header: 'Max Value' },
          { field: 'colNm', header: 'Column Name' },
          { field: 'colValCd', header: 'ColValCode' },
          { field: 'price', header: 'Price' }
        ];
  ...
  getItemDetails(val) {
    ...
    this.tempdBOElectricalListArray = [];
    this._ItoUpdateCostDBOElecPrice.getDBOElectricalData(val).subscribe(res => {
      console.log(res);
      this.tempRes = res;
      this.dBOElectricalListArray = res.dBOElectricalList;
      for (var i = 0; i < this.dBOElectricalListArray.length; i++) {
        if (this.itemNameAlt == this.dBOElectricalListArray[i].catDetDesc) {
          this.tempdBOElectricalListArray.push(this.dBOElectricalListArray[i]);
        }
      }this.dBOElectricalListArray = this.tempdBOElectricalListArray;
    })
  }
  ...
  SaveAsDraft() {
    ...
    for (let q = 0; q < this.finalRangeList.length; q++) {
      if (this.finalRangeList[q].priceId > 0) {
        this.saveBasic.updateCode = "UPD_DBO_ELE_ADDON_PRICE";
      } else {
        this.saveBasic.updateCode = "UPD_DBO_ELE_ADDON_PRICE_NEW";
      }
    }
    ...
    this._ItoUpdDboEleSpladdonService.createDboEleUpdateRequestPriceAddOn(this.tempRes1).subscribe(resp => {
      console.log(resp);
