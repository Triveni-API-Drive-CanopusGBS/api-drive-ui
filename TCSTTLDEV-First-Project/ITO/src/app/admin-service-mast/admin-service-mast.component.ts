...
@Component({
  selector: 'app-admin-service-mast',
  templateUrl: './admin-service-mast.component.html',
  styleUrls: ['./admin-service-mast.component.css']
})
export class AdminServiceMastComponent implements OnInit {
  ...
  constructor(private _ITOturbineConfigService: ITOturbineConfigService, private _ITOScopeOfSupplyService: ITOScopeOfSupplyService,
    ...
    private _ITOeditQoutService: ITOeditQoutService,  @Inject(LOCAL_STORAGE) private storage: WebStorageService) {
      ...
      this._BgmratingsComponentService.getAdminCacheWithAIList().subscribe(resp => {
        console.log(resp);
        this.spinner=true;
        this.tableList=resp.dropDownColumnvalues.placeList19.placeList19;
        for(let j=0;j<this.tableList.length;j++)
        {


           this.cols = [
            { field: 'ssId', header: 'SSID' },
            { field: 'turbineCode', header: 'Turbine Code' },
            { field: 'minPower', header: 'Min Power' },
			{ field: 'maxPower', header: 'Max Power' },
			{ field: 'noOfMandays', header: 'No Of days' },
      { field: 'serviceRemarks', header: 'Service Remarks'},
           ];
          }
  ...
  for(let j=0;j<this.finalFrameList.length;j++)
  {
    this.dboClass = new dboClass();
          ...
          this.dboClass.id=this.finalFrameList[j].id;
				  this.dboClass.ssId=this.finalFrameList[j].ssId;
				  this.dboClass.turbineCode=this.finalFrameList[j].turbineCode;
					this.dboClass.minPower=this.finalFrameList[j].minPower;
					this.dboClass.maxPower=this.finalFrameList[j].maxPower;
          this.dboClass.noOfMandays=this.finalFrameList[j].noOfMandays;
          this.dboClass.serviceRemarks=this.finalFrameList[j].serviceRemarks;
					this.dboClass.activeNew=1;

  this.adminForm.adminServiceMastList.push(this.dboClass);
  }
  this._BgmratingsComponentService.getAdminServiceMast(this.adminForm).subscribe(res => {
    console.log(res);
    this.adminForm = res;
